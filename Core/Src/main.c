...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	    ...
	    if (GPIO_Pin == COL_1_Pin || GPIO_Pin == COL_2_Pin || GPIO_Pin == COL_3_Pin || GPIO_Pin == COL_4_Pin) {
	        // Código para detectar la tecla presionada e ingresarla en claveIngresada
	        // Actualiza la clave ingresada y el display OLED si se conecta
	        if (cindex < 6) {
	            claveIngresada[cindex++] = detectarTecla(GPIO_Pin);  // función para detectar qué tecla fue presionada
	            actualizarDisplay(claveIngresada, cindex);  // Mostrar en OLED
	        }
	    }
...
void mostrarError() {
    ...
    HAL_UART_Transmit(&huart2, errorMsg, sizeof(errorMsg)-1, 100);
    // Parpadeo a 4Hz
    for (int i = 0; i < 12; i++) {
...
void mostrarExito() {
    HAL_GPIO_WritePin(SYSTEM_LED_GPIO_Port, SYSTEM_LED_Pin, GPIO_PIN_RESET);
    HAL_UART_Transmit(&huart2, successMsg, sizeof(successMsg)-1, 100);
}
...
void actualizarDisplay(uint8_t *clave, uint8_t size) {
    // Mostrar la clave ingresada en el OLED
